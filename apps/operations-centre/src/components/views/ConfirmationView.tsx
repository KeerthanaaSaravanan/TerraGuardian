...
export const ConfirmationView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-5 p-4 lg:p-6 w-full max-w-[1600px] mx-auto">
      ...
      {/* CORE OPERATIONAL CALLOUT: ACTION CONFIRMED ≠ HAZARD RESOLVED */}
      <PrincipleBanner
        principle="ACTION CONFIRMED ≠ HAZARD RESOLVED"
        title="Closed-Loop Verification Complete: Public is safe, but slope remains active."
        explanation="Confirmation that the roadblock is physically manned means human exposure is mitigated (Casuality Risk: Zero). However, the physical landslide hazard remains active and will transition to MONITORING state until BRO heavy machinery completes slope stabilization and carriageway clearance."
        variant="emerald"
      />

      {/* Success Confirmation Banner */}
      <div className="bg-gradient-to-r from-emerald-50 via-white to-teal-50 dark:from-emerald-950/80 dark:via-neutral-900 dark:to-teal-950/80 border-2 border-emerald-500 rounded-xl p-6 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="flex items-start gap-4">
          <span className="p-3.5 rounded-2xl bg-emerald-600 text-white shadow-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 p-5 rounded-xl flex flex-col justify-between gap-3 shadow-sm">
          <div>
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-neutral-800 text-[11px] font-mono text-slate-500 dark:text-neutral-500">
            Confirming Officer: <strong className="text-slate-800 dark:text-neutral-300">ASI D. Sonam</strong>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 p-5 rounded-xl flex flex-col justify-between gap-3 shadow-sm">
          <div>
            <div className="text-xs font-mono text-slate-600 dark:text-neutral-400 uppercase font-bold flex items-center gap-2">
              <IconRadio className="w-4 h-4 text-cyan-600 dark:text-cyan-400" />
              EARTHMOVER STAGING
            </div>
            <div className="text-sm font-bold text-slate-900 dark:text-white mt-2">
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-neutral-800 text-[11px] font-mono text-slate-500 dark:text-neutral-500">
            Field Unit: <strong className="text-slate-800 dark:text-neutral-300">Major V. Sharma (BRO)</strong>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 p-5 rounded-xl flex flex-col justify-between gap-3 shadow-sm">
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-neutral-800 text-[11px] font-mono text-slate-500 dark:text-neutral-500">
            Dispatch Node: <strong className="text-slate-800 dark:text-neutral-300">DDMA Control Room</strong>
          </div>
        </div>
      </div>
    </div>
