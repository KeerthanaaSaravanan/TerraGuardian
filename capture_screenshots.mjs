...
async function captureScreenshots() {
  ...
  await page.evaluate(() => localStorage.setItem("terraguardian-theme", "light"));
  ...
  await page.evaluate(() => localStorage.setItem("terraguardian-theme", "dark"));
  await page.reload({ waitUntil: "networkidle" });
  await page.waitForTimeout(300);
  ...
  await page.locator("button[aria-label='Toggle Theme']").first().click();
  await page.waitForTimeout(300);
  await page.screenshot({ path: path.join(ARTIFACT_DIR, "05_review_dark.png"), fullPage: false });
